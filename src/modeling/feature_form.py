...
class CardApprovalFeatures(pydantic.BaseModel):
    ...
    def set_datatypes(self, dataframe: pandas.DataFrame) -> None:
        ...
        dataframe['annual_income'] = dataframe['annual_income'].astype(
            numpy.float32)
        dataframe['credit_window'] = dataframe['credit_window'].astype(
            numpy.float64)

        dataframe['Male'] = dataframe['Male'].astype(numpy.bool_)
        dataframe['Female'] = dataframe['Female'].astype(numpy.bool_)

        dataframe["total_children"] = dataframe["total_children"].astype(
            numpy.int8)
    ...
    def encoded_data(self) -> pandas.DataFrame:
        ...
        if all([self.Male, self.Female]):
            raise ValueError(
                "You need to choose one of the genders, Male or Female")

        if not self.Male and not self.Female:
            raise ValueError(
                "Field Male and Female are required, please, fill one of them")
    ...
    def __standardize_numeric_features(self) -> pandas.DataFrame:
        ...
        scaled_data = pandas.DataFrame(data.reshape(
            1, -1), columns=numeric_features.columns)
        return scaled_data
